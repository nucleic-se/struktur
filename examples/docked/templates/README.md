...
### 1. **Use Triple-Stash for Configs**
Always use `{{{...}}}` for configuration values containing special characters:
```handlebars
✅ command: {{{aspects.docker_container.command}}}
❌ command: {{aspects.docker_container.command}}  # Breaks with special chars
```

### 2. **Conditional Sections**
Wrap optional sections in conditionals to avoid empty blocks:
```handlebars
...
Add comments explaining architecture and patterns:
```handlebars
